...
public:
	...
	AP4_Result    Init() { return m_Samples->GetSample(0, m_Sample); }
...
size_t AP4_Decrypt::decryptAndFragment(uint8_t* segmentData, const size_t segmentSize, const char* keyId, const char* key) const {
	...
	AP4_MemoryByteStream* decryptedOutput = new AP4_MemoryByteStream();
	AP4_CencDecryptingProcessor* processor = new AP4_CencDecryptingProcessor(&keyMap);

	AP4_Result result = processor->Process(*inputBuffer, *decryptedOutput);
	...
	AP4_File input_file(*decryptedOutput, true);
	...
	decryptedOutput->Tell(position);

	if (input_file.GetMovie()->HasFragments()) {
		AP4_LinearReader reader(*input_file.GetMovie(), decryptedOutput);
		for (unsigned int i = 0; i < cursors.ItemCount(); i++) {
	...
	if (auto_detect_fragment_duration) {
		if (video_track) {
			...
			fragment_duration = AutoDetectAudioFragmentDuration(*decryptedOutput, audio_track);
		}
	...
	free(segmentData);
	const uint64_t newSize = output_stream->GetDataSize();
	segmentData = (uint8_t*)malloc(newSize);

	memcpy(segmentData, output_stream->GetData(), newSize);
